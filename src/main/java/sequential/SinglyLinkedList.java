...
public class SinglyLinkedList<T> implements List<T> {
    ...
    public T remove(int index) {
        ...
        // 2. If the index is equal to the size - 1 we need to delete at the end, so we use two pointer one that reach the
        // end of the list and other that was point to the previous element then we change the tail to point to the previous
        // 3. Delete an element between the beginning and the end of the list, we need two pointers one that references to
        // current element to be deleted and another that references the previous, then point the next pointer from the previous
        // to the next pointer of the current as the current is not referenced by any it will be freed up by the garbage collector
        Node<T> temp;
        if(index == 0) {
            temp = this.head;
